"""Kaggle bootstrap for one immutable Qwen3.5 D1-D4 diagnostic."""
...
def main() -> None:
    ...
    try:
        ...
        expected = {
            ...
            for key in (
                "diagnostic_config_path", "runtime_path", "design_path", "model_config_path",
                "prompt_config_path", "bundle_path", "protocol_path", "override_lock_path",
        ...
        _run([
            ...
            "--require-hashes", "-r", str(source / spec["qwen35_override_lock"]),
        ], source)
        phase = "D1_D4_measurement_diagnostic"
        spec_path = Path("/tmp/labbs2026-qwen35-measurement-spec.json")
        _atomic_json(spec_path, spec)
        _run([str(venv_python), "-m", "labbs2026.stage0.measurement_diagnostic", "--remote-spec", str(spec_path)], source)
