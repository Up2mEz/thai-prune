"""Immutable Kaggle bootstrap for the authorized Paddle/Wayu smoke."""
    ...
    try:
        ...
        _run(
            [
                ...
                "--require-hashes",
                "-r",
                str(source / spec["transformers_override_lock"]),
            ],
            source,
        )
        phase = "authorized_engineering_smoke"
        spec_path = Path("/tmp/labbs-paddle-wayu-smoke-spec.json")
        ...
        message = re.sub(
            r"(?i)(token|key|password)=\S+", r"\1=<redacted>", str(exc)
        )
