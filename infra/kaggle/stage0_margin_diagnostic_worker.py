"""Kaggle bootstrap for one immutable Stage 0 margin diagnostic."""
...
def main() -> None:
    ...
    try:
        ...
        expected = {
            ...
            for key in (
                "diagnostic_config_path", "config_path", "runtime_path",
                "model_config_path", "prompt_config_path", "bundle_path",
        ...
        phase = "stage0_margin_diagnostic"
        spec_path = Path("/tmp/labbs2026-stage0-margin-diagnostic-spec.json")
        _atomic_json(spec_path, spec)
        _run([
            str(source / ".venv/bin/python"), "-m",
            "labbs2026.stage0.margin_diagnostic", "--remote-spec", str(spec_path),
        ], source)
