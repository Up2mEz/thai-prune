"""Thin CLI for the pre-registered Qwen3.5 D1-D4 measurement diagnostic."""
...
CONFIG = ROOT / "configs/stage0/qwen35_measurement_diagnostic.yaml"
...
def main() -> int:
    ...
    if args.command == "preflight":
        result = diagnostic_preflight(ROOT, CONFIG, RUNTIME)
        _print(result)
        return 0 if result["valid"] else 1
    if args.command == "prepare":
        _print(prepare_staging(ROOT, CONFIG, RUNTIME))
        return 0
    ...
    if args.command == "analyze":
        ...
        _print(analyze_artifacts(artifact_dir, run_dir / "analysis", CONFIG))
        return 0
    status = query_status(submission["kernel_id"], ROOT)
    ...
    if args.command == "fetch":
        if status["returncode"] or status["status"] != "COMPLETE":
            _print(status)
            return 1
        destination, temporary = run_dir / "fetched", run_dir / ".fetch.tmp"
    ...
    artifact_dir = run_dir / "fetched/artifacts" / submission["run_id"]
    result = verify_artifacts(artifact_dir, submission, status_record["status"])
