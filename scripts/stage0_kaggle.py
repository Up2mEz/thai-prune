"""Thin CLI for frozen Stage 0 calibration submission and verification."""
...
CONFIG = ROOT / "configs/stage0/calibration_design.yaml"
RUNTIME = ROOT / "configs/runtime/kaggle_t4.yaml"
MODEL = ROOT / "configs/stage0/qwen25_vl_3b_calibration.yaml"
...
def main() -> int:
    ...
    for name in ("submit-command", "status", "fetch", "verify"):
        command = subparsers.add_parser(name)
        command.add_argument("--run-dir", type=Path, required=True)
    args = parser.parse_args()
    if args.command == "preflight":
        result = stage0_preflight(ROOT, CONFIG, RUNTIME, MODEL)
        _print(result)
        return 0 if result["valid"] else 1
    if args.command == "prepare":
        result = prepare_stage0_staging(ROOT, CONFIG, RUNTIME, MODEL)
        _print(result)
        return 0
    ...
    result = verify_stage0_artifacts(artifact_dir, submission, status_record["status"])
