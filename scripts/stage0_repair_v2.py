"""Thin CLI for Stage 0 Calibration Repair v2."""
...
from labbs2026.stage0.repair_v2 import (
    ...
    query_kaggle_status,
    repair_preflight,
    verify_repair_artifacts,
)

ROOT = Path(__file__).resolve().parents[1]
REPAIR = ROOT / "configs/stage0/calibration_repair_v2.yaml"
...
def main() -> int:
    ...
    if args.command == "preflight":
        result = repair_preflight(ROOT, REPAIR, RUNTIME)
        _print(result)
        return 0 if result["valid"] else 1
    if args.command == "prepare":
        result = prepare_repair_staging(ROOT, REPAIR, RUNTIME)
        _print(result)
        return 0
    ...
    result = verify_repair_artifacts(
        artifact_dir, submission, status_record["status"]
    )
